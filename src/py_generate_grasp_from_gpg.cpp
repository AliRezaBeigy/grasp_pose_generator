...
PYBIND11_MODULE(grasp_pose_generator, m) {
    ...
    py::class_<Grasp>(m, "Grasp")
            ...
            .def("set_score", &Grasp::setScore, "set grasp score")
            .def("is_full_antipodal", &Grasp::isFullAntipodal, "check if grasp is full antipodal")
            .def("is_half_antipodal", &Grasp::isHalfAntipodal, "check if grasp is half antipodal")
            .def("set_full_antipodal", &Grasp::setFullAntipodal, "set full antipodal flag")
            .def("set_half_antipodal", &Grasp::setHalfAntipodal, "set half antipodal flag")
            .def("get_sample", &Grasp::getSample, "get sample point")
            .def("get_frame", &Grasp::getFrame, "get grasp frame")
            .def("get_position", &Grasp::getPosition, "get grasp position")
            .def("get_center", &Grasp::getCenter, "get center position")
            .def("get_bottom", &Grasp::getBottom, "get bottom position")
            .def("get_left", &Grasp::getLeft, "get left position")
            .def("get_right", &Grasp::getRight, "get right position")
            .def("get_top", &Grasp::getTop, "get top position")
            .def("get_finger_placement_index", &Grasp::getFingerPlacementIndex, "get finger placement index")
            .def("set_grasp_width", &Grasp::setGraspWidth, "set grasp width")
            .def("set_grasp_bottom", &Grasp::setGraspBottom, "set grasp bottom")
            .def("set_grasp_surface", &Grasp::setGraspSurface, "set grasp surface")
            .def("set_grasp_top", &Grasp::setGraspTop, "set grasp top")
            .def("print", &Grasp::print, "print grasp information")
      ;
}
